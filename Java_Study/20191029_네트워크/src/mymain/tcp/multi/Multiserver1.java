		...
		public void run() {
			...
			while (true) {
				try {
					...
					} else if (msgs[0].equals("MSG")) { // 채팅이면
						synchronized (syncObj) {
					...
					} else if (msgs[0].equals("DRAW")) { // 그림판 데이터면
						synchronized (syncObj) {
							send_message_all(readStr + "\n");
						}
				...
				} catch (IOException e) { // 상대방쪽에서 비정상으로 종료했을 때 (ex.와이파이x, 종료버튼이 아닌 닫기 버튼 눌렀을 때)
